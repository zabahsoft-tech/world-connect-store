// Build the project for Node.js (cPanel) and zip it into a single uploadable
// artifact at /mnt/documents/cpanel-deploy.zip.
//
// Usage:  node scripts/package-cpanel.mjs   (or:  npm run package:cpanel)

import { spawnSync } from "node:child_process";
import { existsSync, mkdirSync, readdirSync, statSync, createWriteStream, readFileSync } from "node:fs";
...
const OUT_ZIP = join(OUT_DIR, "cpanel-deploy.zip");

function log(msg) {
  console.log(`\x1b[36m[package:cpanel]\x1b[0m ${msg}`);
}
function fail(msg) {
  console.error(`\x1b[31m[package:cpanel]\x1b[0m ${msg}`);
  process.exit(1);
}
...
// 1. Build
// ---------------------------------------------------------------------------
log("Running `vite build` with BUILD_TARGET=node ...");
const build = spawnSync(
  ...
  ["vite", "build"],
  {
    cwd: ROOT,
    stdio: "inherit",
    env: { ...process.env, BUILD_TARGET: "node", NODE_ENV: "production" },
  },
);
...
if (!existsSync(join(ROOT, "dist", "server", "server.js"))) {
  fail(
    "Build finished but dist/server/server.js is missing. " +
      "Check vite.config.ts for the BUILD_TARGET=node branch.",
  );
}

// ---------------------------------------------------------------------------
// 2. Collect files for the zip
// ---------------------------------------------------------------------------
const includeRoots = ["dist", "server.mjs", "package.json", "package-lock.json", ".env.example", "README-cpanel.md"];
const excludeNames = new Set(["node_modules", ".git", ".DS_Store"]);
...
for (const r of includeRoots) {
  ...
  if (!existsSync(abs)) {
    if (r === "package-lock.json") {
      log("(skipping package-lock.json — not present)");
      continue;
    }
    fail(`Required path missing: ${r}`);
  }
  walk(abs, r, files);
}
...
log("Upload this zip to your cPanel app folder, extract it, then click 'Run NPM Install' and 'Restart'.");
